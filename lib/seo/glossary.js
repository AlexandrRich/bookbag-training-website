export const glossaryTerms = {
  "ai-production-gate": {
    ...
    faqs: [
      ...
      { question: "Does a production gate slow down delivery?", answer: "Not meaningfully. Messages that pass your rubric get the safe_to_deploy verdict and auto-approve instantly — no human touch needed. Only needs_fix and blocked items enter the review queue. After calibration, most teams see 70-85% auto-approval rates. The 15-30% that need human review are exactly the ones you want a human looking at." },
  ...
  "message-gating": {
    ...
    howBookbagHelps: "Bookbag's gating system routes every message through three lanes: safe_to_deploy (auto-approved, no delay), needs_fix (QA reviewer corrects and approves), and blocked (SME reviews with documented rationale). Every verdict carries rubric citations, reviewer identity, and timestamps. The gating data feeds directly into training data export — so every correction makes the next batch of messages better.",
    ...
    faqs: [
      ...
      { question: "Can gating handle high volume?", answer: "That's the whole point. The safe_to_deploy lane auto-approves clean messages with zero delay. Only the 15-30% that need human authority actually enter the review queue. Bookbag scales gating to millions of messages per month without bottlenecking delivery." },
    ],
  ...
  "safe-to-deploy": {
    ...
    whyItMatters: "safe_to_deploy rates are your north star metric for AI outbound efficiency. Low safe rates mean your gate is a bottleneck — too many messages need human review. High safe rates mean your AI is well-calibrated, your rubrics are dialed in, and your review costs are dropping. Most teams start at 50-60% and reach 70-85% after calibrating with real correction data. That's the flywheel working.",
    howBookbagHelps: "Bookbag's safe_to_deploy lane auto-approves messages that clear your rubric — zero human delay. But every safe message is still logged with full provenance: which rubric version applied, when the verdict was rendered, what the message contained. QA can sample safe messages anytime for calibration checks. And as your team exports corrections as training data and retrains models, safe rates climb over time. That's less review cost, faster delivery, and a growing immutable audit trail.",
    bookbagFeatures: [
      { title: "Zero-delay auto-approval", description: "Messages meeting all rubric criteria get safe_to_deploy instantly — no queue, no human wait time, no bottleneck." },
    ...
    faqs: [
      ...
      { question: "What percentage of messages are typically safe?", answer: "After initial calibration, most teams land at 70-85% safe_to_deploy rates. Regulated industries often start lower — maybe 50-60% — because their rubrics are stricter. But as corrections from needs_fix items become training data, safe rates climb. That's the flywheel doing its job." },
  ...
  "ai-message-quality": {
    ...
    faqs: [
      ...
      { question: "What's a 'good' quality rate?", answer: "After calibration, most teams target 70-85% safe_to_deploy rates — meaning that percentage of AI messages meet all quality standards without human intervention. Starting rates are usually 50-60%. The gap between starting and target is where the training data flywheel does its work." },
  ...
  "human-in-the-loop-ai": {
    ...
    faqs: [
      { question: "Doesn't human-in-the-loop slow things down?", answer: "Only for the messages that need it — and those are exactly the ones you want a human looking at. safe_to_deploy messages auto-approve instantly with zero human delay. Humans focus on the 15-30% that are needs_fix or blocked. The result is actually faster than reviewing everything manually, because 70-85% of messages skip the human queue entirely." },
